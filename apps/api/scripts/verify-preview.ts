...
async function main() {
	...
	try {
		...
		if (
			...
			console.log(
				"⚠️ Failed to create product (Subject ID issue). Fetching subjects...",
			);
			// Fetch subjects
			try {
				const subjectsRes = await axios.get(
					`${API_URL}/catalog/subjects?limit=1`,
				);
				...
				console.error(
					"❌ Failed to create product even after retry:",
	...
	page.drawText("This is a test PDF for preview generation.", {
		x: 50,
		y: 700,
		size: 30,
	});
